...
class WhoToCountAgainstTest {
  ...
  private final WhoToCountAgainst counting = new WhoToCountAgainst();
  ...
  void noAddressIsStillCounted() {
    ...
    StepVerifier.create(counting.resolve(exchange))
        .expectNext(WhoToCountAgainst.NOBODY_IN_PARTICULAR)
        .verifyComplete();
  }
  ...
  void aBlankNameDoesNotBecomeAnEmptyKey() {
    ...
    assertThat(WhoToCountAgainst.nameOf(blank)).isEqualTo(WhoToCountAgainst.NOBODY_IN_PARTICULAR);
  }
  ...
  void aRealNameIsUsedAsItStands() {
    ...
    assertThat(WhoToCountAgainst.nameOf(named)).isEqualTo("user11");
  }

  private MockServerWebExchange anExchangeFrom(String address) {
    return MockServerWebExchange.from(
        MockServerHttpRequest.get("/api/funds")
            .remoteAddress(new java.net.InetSocketAddress(address, 51234)));
  }
