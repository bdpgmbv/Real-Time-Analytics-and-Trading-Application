...
@Service
public class FundFacts {

  private final JdbcTemplate database;
  private final Sql sql;
  private final Counter timesWeActuallyAsked;

  public FundFacts(JdbcTemplate database, Sql sql, MeterRegistry meters) {
    this.database = database;
    this.sql = sql;
    this.timesWeActuallyAsked = meters.counter("rtat.reference.read.from.database");
  }

  @Cacheable(WhatWeCache.FUND_REPORTING_CURRENCY)
  ...
  @Cacheable(WhatWeCache.ACCOUNTS_IN_FUND)
