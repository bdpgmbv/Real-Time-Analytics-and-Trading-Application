...
class WhatWeCacheTest {
  ...
  private final CacheManager caches = new WhatWeCache().whatWeKeep(1000, 300);
  ...
  void theThreeRepeatedReadsAreKept() {
    assertThat(caches.getCacheNames())
        .containsExactlyInAnyOrder(
            WhatWeCache.EXCHANGE_RATES,
            WhatWeCache.FUND_REPORTING_CURRENCY,
            WhatWeCache.ACCOUNTS_IN_FUND);
  }
  ...
  void aRatePutInComesBackOut() {
    Cache rates = cacheCalled(caches, WhatWeCache.EXCHANGE_RATES);
  ...
  void aStaleRateIsNotServed() throws InterruptedException {
    Cache rates = cacheCalled(new WhatWeCache().whatWeKeep(50, 300), WhatWeCache.EXCHANGE_RATES);
  ...
  void referenceDataOutlivesRates() throws InterruptedException {
    CacheManager mixed = new WhatWeCache().whatWeKeep(50, 300);
    Cache rates = cacheCalled(mixed, WhatWeCache.EXCHANGE_RATES);
    Cache accounts = cacheCalled(mixed, WhatWeCache.ACCOUNTS_IN_FUND);
