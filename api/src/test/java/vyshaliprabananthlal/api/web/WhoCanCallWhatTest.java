...
class WhoCanCallWhatTest {
  ...
  void aRubbishTokenIsRefused() throws Exception {
    ...
    org.mockito.Mockito.verify(entitlements, org.mockito.Mockito.never())
        .fundsVisibleTo(anyString());
  }
  ...
  void aValidTokenGetsTheirOwnFunds() throws Exception {
    when(entitlements.fundsVisibleTo("adriatic-reader"))
        .thenReturn(List.of(new VisibleFund(10, "Adriatic Growth", "USD", false)));
  ...
  void accountsAreNarrowed() throws Exception {
    ...
    http.perform(
            get("/api/funds/10/exposure?account=100&account=200")
                .with(jwt().jwt(it -> it.subject("adriatic-reader"))))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.howManyAccounts").value(1));
