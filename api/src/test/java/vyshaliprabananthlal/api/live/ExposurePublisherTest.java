...
class PushToTheScreensTest {
    ...
    void oneBadFundDoesNotStopTheRest() {
        when(watching.watchedFunds()).thenReturn(new java.util.TreeSet<>(Set.of(1, 2)));
    ...
    void everyWatchedFundIsPushed() {
        when(watching.watchedFunds()).thenReturn(new java.util.TreeSet<>(Set.of(1, 2, 3)));
    ...
    private FundExposure exposureOf(String currency, double amount) {
        return new FundExposure(1, "USD", List.of(new FundExposure.CurrencyAmount(currency, amount, amount)), 2);
    }
}
