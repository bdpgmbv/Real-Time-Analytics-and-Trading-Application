...
public class KafkaBatch {
    ...
    public int writeThenAcknowledge(String feed, String statement, List<Object[]> rows, Acknowledgment kafka) {
    ...
    private Timer writeTimer(String feed) {
        return writeTimers.computeIfAbsent(feed, which -> Timer.builder("rtat.batch.write")
                .tag("feed", which)
    ...
    private Counter rowCounter(String feed) {
        return rowCounters.computeIfAbsent(
                feed,
                which -> Counter.builder("rtat.rows.changed").tag("feed", which).register(meters));
    }

    private Counter failureCounter(String feed) {
        return failureCounters.computeIfAbsent(
                feed,
                which -> Counter.builder("rtat.batch.failed").tag("feed", which).register(meters));
    }
