...
class HedgeFillListenerTest {
    ...
    void startWithOneHedgeWaitingToBeFilled() {
        ...
        database.execute("INSERT INTO hedge (hedge_id, fund_id, currency, hedge_date, exposure_amount,"
    ...
    void aFullFillCompletesTheHedge() {
        listener.arrived(List.of(aFill(1, 9000000, 1.1540)), kafka);
    ...
    void aPartialFillLeavesItOpen() {
        listener.arrived(List.of(aFill(1, 6000000, 1.1540)), kafka);
    ...
    void twoFillsTogetherCompleteIt() {
        ...
        listener.arrived(List.of(aFill(2, 3000000, 1.1538)), kafka);
    ...
    void eachFillKeepsItsOwnRate() {
        listener.arrived(List.of(aFill(1, 6000000, 1.1540), aFill(2, 3000000, 1.1538)), kafka);

        List<Double> rates =
                database.query("SELECT fill_rate FROM hedge_fill ORDER BY fill_id", (row, number) -> row.getDouble(1));
    ...
    private double totalFilled() {
        Double total = database.queryForObject("SELECT coalesce(sum(amount_filled), 0) FROM hedge_fill", Double.class);
    ...
    private static String readFile(String path) {
        try (InputStream stream = HedgeFillListenerTest.class.getClassLoader().getResourceAsStream(path)) {
            if (stream == null) {
