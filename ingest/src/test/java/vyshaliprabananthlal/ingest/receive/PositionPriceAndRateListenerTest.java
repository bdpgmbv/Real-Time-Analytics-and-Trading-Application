...
class PositionPriceAndRateListenerTest {
    ...
    void startWithSomeData() {
        ...
        database.execute(
                "TRUNCATE trade, position, price, fx_rate, account, fund, client, product," + " currency CASCADE");
        database.execute("INSERT INTO currency VALUES ('USD', 'US Dollar', 2), ('JPY', 'Japanese Yen', 0)");
        database.execute("INSERT INTO fx_rate VALUES ('JPY', 'USD', 0.006336, CURRENT_DATE, 'STARTING VALUE')");
        ...
        database.execute("INSERT INTO price (product_id, price, freshness, price_date, arrived_at)"
                + " SELECT product_id, 100.0, 'DELAYED 20 MINUTES', CURRENT_DATE, now() FROM product");
        database.execute("INSERT INTO position (account_id, product_id, quantity, cost,"
    ...
    void positionIsSetNotAdded() {
        Listeners.Positions listener = new Listeners.Positions(
                new KafkaBatch(database, new SimpleMeterRegistry()), new JsonReader(), new Sql());

        listener.arrived(List.of(positionMessage(750)), kafka);
    ...
    void priceIsUpdated() {
        Listeners.Prices listener =
                new Listeners.Prices(new KafkaBatch(database, new SimpleMeterRegistry()), new JsonReader(), new Sql());

        listener.arrived(List.of(priceMessage(250.75)), kafka);
    ...
    void rateIsUpdatedAndMarked() {
        Listeners.Rates listener =
                new Listeners.Rates(new KafkaBatch(database, new SimpleMeterRegistry()), new JsonReader(), new Sql());

        listener.arrived(List.of(rateMessage("JPY", "USD", 0.0064)), kafka);
    ...
    void unknownRatePairIsIgnored() {
        ...
        listener.arrived(List.of(rateMessage("ZZZ", "USD", 1.23)), kafka);
    ...
    private String positionMessage(long howMany) {
        return String.format("{\"accountId\":%d,\"productId\":%d,\"howMany\":%d}", accountId(), productId(), howMany);
    }

    private String priceMessage(double price) {
        return String.format("{\"productId\":%d,\"price\":%s,\"howFresh\":\"DELAYED 20 MINUTES\"}", productId(), price);
    }
