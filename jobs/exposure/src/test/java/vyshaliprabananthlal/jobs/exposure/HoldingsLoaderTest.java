...
class LoadWhoHoldsWhatTest {
    ...
    static void twoFundsHoldingTheSameSecurity() throws SQLException {
        ...
        run("INSERT INTO currency VALUES ('USD','US Dollar',2),('EUR','Euro',2);"
    ...
    void accountsInAFundAreRolledUp() throws SQLException {
        Map<Integer, List<ExposureMessages.FundHolding>> whoHoldsWhat = load();

        List<ExposureMessages.FundHolding> holders = whoHoldsWhat.get(500);

        assertThat(holders).hasSize(2);
        assertThat(holders).extracting(ExposureMessages.FundHolding::quantity).containsExactlyInAnyOrder(10.0, 100.0);
    }
    ...
    void eachFundAppearsOnce() throws SQLException {
        assertThat(load().get(500))
                .extracting(ExposureMessages.FundHolding::fundId)
    ...
    private Map<Integer, List<ExposureMessages.FundHolding>> load() throws SQLException {
    ...
    private static String readFile(String path) {
        try (InputStream stream = LoadWhoHoldsWhatTest.class.getClassLoader().getResourceAsStream(path)) {
            if (stream == null) {
