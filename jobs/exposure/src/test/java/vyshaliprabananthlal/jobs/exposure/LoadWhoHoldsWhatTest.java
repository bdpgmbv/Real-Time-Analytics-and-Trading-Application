...
class LoadWhoHoldsWhatTest {
  ...
  void accountsInAFundAreRolledUp() throws SQLException {
    Map<Integer, List<WhoHoldsIt>> whoHoldsWhat = load();

    List<WhoHoldsIt> holders = whoHoldsWhat.get(500);

    assertThat(holders).hasSize(2);
    assertThat(holders).extracting(WhoHoldsIt::howMany).containsExactlyInAnyOrder(10.0, 100.0);
  }
  ...
  void eachFundAppearsOnce() throws SQLException {
    assertThat(load().get(500)).extracting(WhoHoldsIt::fundId).containsExactlyInAnyOrder(10, 11);
  }
  ...
  void theCountIsTheWorkAPriceTickCauses() throws SQLException {
    assertThat(LoadWhoHoldsWhat.howManyHoldings(load())).isEqualTo(2);
  }
